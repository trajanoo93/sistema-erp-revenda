...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($cliente_id) || empty($data_retirada) || empty($produtos) || empty($quantidades)) {
        registrarLog("Erro: Campos obrigatórios não preenchidos. cliente_id=" . ($cliente_id ?: 'vazio') . ", data_retirada=" . ($data_retirada ?: 'vazio') . ", produtos=" . (empty($produtos) ? 'vazio' : count($produtos)) . ", quantidades=" . (empty($quantidades) ? 'vazio' : count($quantidades)));
    ...
    if (count($produtos) !== count($quantidades)) {
        registrarLog("Erro: Inconsistência nos dados de produtos. Número de produtos (" . count($produtos) . ") e quantidades (" . count($quantidades) . ") não coincidem. Usuário ID: {$_SESSION['usuario_id']}");
    ...
    try {
        ...
        if ($stmtPedido->execute()) {
            ...
            foreach ($produtos as $index => $produto_id) {
                $quantidade = str_replace(',', '.', $quantidades[$index]);
                // Log da quantidade recebida
                registrarLog("Processando produto ID: $produto_id, Quantidade bruta: {$quantidades[$index]}, Quantidade formatada: $quantidade");
                ...
                if ($produto) {
                    ...
                    if ($tipo === "UND") {
                        if (!ctype_digit(strval($quantidade)) || (int)$quantidade <= 0) {
                        ...
                        if (!is_numeric($quantidade) || $quantidade <= 0 || !preg_match('/^\d{1,3}(?:\.\d{0,3})?$/', $quantidade)) {
